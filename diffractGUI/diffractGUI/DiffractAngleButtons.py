...
def set_up():
    ...
    def increment_angle(index):
        angles[index] += 0.1
        print(f"angle {i} is {angles[index]:5.5}")
    ...
    i = 0  # Define the i variable here

    return (set_up)

def main_loop(set_up):
    set_up()
    while not done:
                ...
                for i in range(len(button_positions)):
                    if check_button_click(mouse_pos, i):
                        ...
                        if i % 2 == 0:
                            ...
                            for i, position in enumerate(button_positions, button_padding):
                                button_x, button_y = position
                                button_rect = pygame.Rect(button_x, button_y, button_width, button_height)
                                button_color_to_use = (
                                    button_hover_color
                                    if check_button_click(pygame.mouse.get_pos(), i)
                                    else button_color
                                    )
                                ...
                                button_text = font.render(
                                    f"{button_labels[i]}: {angles[i // 2]:.2f}", True, (0, 0, 0)
                                    )
